...
def main():
    ...
    # Именно `isfile`, а не `exists`: папка с таким именем — не метка, и трактовать
    # ворота иначе, чем сторож расхождения версий, нельзя. У двух хуков одного комплекта
    # ворота объявлены одни (`hooks.json`), значит они обязаны совпадать и на краях.
