...
def main():
    ...
    базу_спрашивали = база_названа or not (args.check_anon or core_files)
    видимые_md = []
    base_files = []
    ...
    if dupes:
        ...
        for key, fs in sorted(dupes.items()):
            print(f"    • «{key}»: " + ", ".join(fs))
        print("    Свести так: выбрать дом (обычно старший по `created`), перенести в него")
        print("    содержимое второй ДОСЛОВНО, второе имя оставить `alias`-ом, второй файл")
        print("    удалить, входящие ссылки на него переставить грепом по имени (§ 6 п. 3).")
