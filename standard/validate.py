...
def main():
    ...
    elif not args.check_anon and not (core_files and not база_названа):
        # Папки базы нет — а именно она и есть предмет проверки. Прежде это прощалось,
        # если задан `--core`: прогон шёл по одному ядру и печатал «📊 База: 0 заметок»
        # и следом «🟢 Ошибок нет». Опечатка в имени папки (`kknowledge`) давала зелёный
        # экран при нуле проверенных заметок, и единственной приметой был ноль в шапке —
        # то есть невыполненная проверка была неотличима от пройденной. Ноль заметок
        # в существующей папке — законное состояние новой базы; НЕСУЩЕСТВУЮЩАЯ папка —
        # всегда ошибка команды.
        print(f"ОШИБКА: папка базы {args.base_dir}/ не найдена — проверять нечего.")
        print("    Это не «в базе всё чисто»: ни одна заметка не проверена. Часто это")
        print("    опечатка в имени папки в команде прогона — сверь её с тем, что на диске.")
        return 2
    elif not args.check_anon:
        # Папка базы не названа, зато названо ядро: прогон по одному ядру — законная
        # работа. Но сказать об этом надо вслух: отчёт без этой строки отличается от
        # полного только ОТСУТСТВИЕМ раздела про базу, а отсутствие никто не замечает.
        print("ℹ️  Папка базы не названа: проверялись ТОЛЬКО документы ядра.")
        print("    О заметках базы этот прогон не говорит ничего.\n")
        ...
        if trust_layer and is_knowledge(note_type[base], args.knowledge_type):
            ...
            if not has_sources:
                ...
                fm_problems.append(
                    ...
                    f"Источника пока нет — не меняй type, а поставь карантин: "
                    f"status: draft (core.md § 10.2)")
