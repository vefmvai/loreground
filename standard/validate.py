...
def status_marker_re(markers) -> re.Pattern:
    ...
    верификатор закрытия 2026-07-30: соседняя проверка того же файла уже снимала
...
def link_names(value) -> list:
    ...
    for item in value:
        ...
        names.append(m.group(1).strip() if m else item.strip())
...
def root_ids(data) -> set:
    ...
    # что класс закрыт. Первая редакция брала только регистр, и `reg-2024-017` против
...
def is_archived(text: str) -> bool:
    if ARCHIVE_STATUS_RE.search(text):
        return True
    head = "\n".join(text.splitlines()[:ARCHIVE_HEAD_LINES])
    return bool(ARCHIVE_HEAD_HINT_RE.search(head))
...
def main():
            ...
            if enum_value(data, "consensus") == "confirmed":
                ...
                if roots < 2:
                    consensus_problems.append(
                        ...
                        f"нужно ≥2 (sources схлопнуты по same_root и общему root_id)")
    ...
    for f in scan_files if archived_names else ():
        ...
        for i, line in enumerate(texts[f].splitlines(), 1):
            if ARCHIVE_LINK_OK_RE.search(line):
                continue
    ...
    for f in base_files:
        ...
        if not isinstance(data, dict):
            continue
        missing = [k for k in DECLARED_FRONTMATTER_KEYS if k not in data]
        if missing:
