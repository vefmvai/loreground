...
отдельной строкой; несуществующий путь — тоже. Аргументом принимается и папка, и один
файл, как у двух соседних переборов.
...
UTF-8, а не локаль (машины под это в комплекте нет: перебор их просто не смотрит), и внесение их сюда красило бы исправный код.
"""
import ast
import os
import re
...
def двоичный(вызов, место=1):
    """Режим с `b`: текста нет, кодировке взяться неоткуда.

    `место` — где в вызове стоит режим. У встроенного `open(файл, режим)` он второй,
    у `Path(...).open(режим)` — первый, потому что имя файла уже в самом объекте.
    Без этой разницы честный `Path("з").open("rb")` объявлялся бы находкой.
    """
    for арг in list(вызов.args[место:место + 1]) + [k.value for k in вызов.keywords if k.arg == "mode"]:
...
def пути_объекты(дерево, карта):
    """Имена переменных, в которые положили `Path(...)`.

    Нужны, чтобы `п = Path("з.md")` и следом `п.open()` не проехало мимо. Судить по
    одному лишь имени метода нельзя: `.open()` есть и у архива, и у браузера, и у
    файлового дескриптора — там кодировке взяться неоткуда, и красный был бы ложным.
    """
    имена = set()
    for узел in ast.walk(дерево):
        if isinstance(узел, ast.Assign) and isinstance(узел.value, ast.Call):
            if развернуть(имя_узла(узел.value.func), карта) == "pathlib.Path":
                имена |= {ц.id for ц in узел.targets if isinstance(ц, ast.Name)}
    return имена


def это_путь(узел, карта, пути):
    """Получатель вызова — объект `pathlib.Path`: прямо в строке либо через переменную."""
    if isinstance(узел, ast.Call):
        return развернуть(имя_узла(узел.func), карта) == "pathlib.Path"
    return isinstance(узел, ast.Name) and узел.id in пути


def разобрать(текст, путь, сдвиг, находки, целиком=False):
    """Разбор одного куска кода. Не разобралось — говорим об этом, а не молчим.

    `целиком` — это весь файл (`.py`), а не выдержка из прозы. Тогда молчать нельзя
    никогда: непрочитанный файл комплекта делает пустой вывод перебора неотличимым от
    ноля дефектов. У выдержки из `.md`/`.sh` порог выше — она бывает намеренно неполной
    (три точки вместо тела), и красный на ней выключил бы стража за шум.
    """
    try:
        дерево = ast.parse(текст)
    except SyntaxError as err:
        if целиком:
            находки.append((путь, сдвиг + (err.lineno or 1) - 1,
                            f"файл не разобрался ({err.msg}) — проверить нечем"))
        elif any(п in текст for п in ПОВОД):
            находки.append((путь, сдвиг, "код с обращением к тексту не разобрался — проверить нечем"))
        return
    карта = псевдонимы(дерево)
    пути = пути_объекты(дерево, карта)
    ...
    for узел in ast.walk(дерево):
        ...
        if полное in ЗАПУСК_ВСЕГДА:
            ...
            continue
        путь_open = (isinstance(узел.func, ast.Attribute) and узел.func.attr == "open"
                     and это_путь(узел.func.value, карта, пути))
        if полное in ЧТЕНИЕ_ТЕКСТА or путь_open:
            if not двоичный(узел, 0 if путь_open else 1) and not есть(узел, "encoding"):
...
def пути_обхода(корень):
    """Файлы для просмотра. Аргументом принимается и папка, и ОДИН файл.

    Раньше умели только папку: переданный файл давал ноль просмотренных файлов и
    пустой вывод с кодом 0 — то есть ровно «путь пришёл, проверять нечего, ответ как
    на успех». Тот самый класс, который стережёт `gate-sweep.py` рядом, — в стороже.
    Два соседних перебора файл принимают, и разнобой сам по себе ловушка: команда,
    работающая с двумя из трёх, читается как работающая со всеми тремя.
    """
    ...
    for папка, подпапки, файлы in os.walk(корень):
        ...
        собрано += [os.path.join(папка, имя) for имя in sorted(файлы)]
    return собрано
...
def main():
    ...
    if not os.path.exists(корень):
        print(f"{корень}:1  путь не найден — просмотрено 0 файлов, и это не «чисто»")
        return 0
    for путь in пути_обхода(корень):
        ...
        for кусок, сдвиг in куски(путь, текст):
            разобрать(кусок, путь, сдвиг, находки, целиком=путь.endswith(".py"))
    основа = корень if os.path.isdir(корень) else os.path.dirname(корень) or "."
    for путь, строка, что in находки:
        print(f"{os.path.relpath(путь, основа)}:{строка}  {что}")
    return 0
