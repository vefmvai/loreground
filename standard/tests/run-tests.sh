...
#
# Случаев ДВА, и одной фикстурой они не проверяются — это выяснилось мутацией. Канон
# нужен и на `sources`, и на группах, а фикстура ловит только тот, чьё имя разошлось с
# именем файла. Первая редакция теста имела alias только в `sources`, и группа совпадала
# с базой файла и без канона — тест зеленел бы и на невылеченном коде.
...
# Настоящая копия — это ВЕСЬ блок, а не одна строка про версию. Прежние фикстуры несли
# только строку версии, то есть были ровно той заглушкой, которую сторож обязан ловить:
# они проверяли сверку номера на файле, где кодекса нет ни строки. Тест зеленел, не
# воспроизводя того, что проверяет, — тот же класс, что уже ловился здесь однажды.
