...
# Строка копии берётся ЖИВОЙ из codex.md, а не сочиняется здесь. Прежняя фикстура была
# голым «версия кодекса N» — это проза, а не копия, и сторож принимал её за копию. Тест
# на этой фикстуре был ложно-зелёным: он не воспроизводил того, что проверяет.
...
# Порядок источников: свой стандарт изделия старше комплекта рядом. Спроси сначала
...
PY
printf 'собран_на_версии: 0\nимя: тест\n' > "$THK2/.loreground"
printf '# Правила\n\n%s\n' "$(copyline 777)" > "$THK2/CLAUDE.md"
OUT=$( (cd "$THK2" && CLAUDE_PROJECT_DIR="$THK2" CLAUDE_PLUGIN_ROOT="$KIT" python3 "$KIT/hooks/codex-freshness.py") )
if [ -z "$OUT" ]; then ok "хук: свой стандарт изделия старше комплекта рядом"; else
...
# Расхождение версии сборки с версией комплекта обязано звучать: обновление изделия —
...
# Комплекта на машине нет — штатное состояние автономного изделия, а не дефект.
...
if [ -z "$OUT" ]; then ok "версии: без комплекта рядом молчит (сравнивать не с чем)"; else
  bad "версии: ругается на автономное изделие, у которого завода на машине нет"; fi

OUT=$(vd 1); RC=$?
has "версии: расхождение названо" "собрано на версии 1.0.0"
has "версии: названа версия комплекта" "9.9.9"
has "версии: без своего стандарта сказано, что правила уже сменились" "работает прямо сейчас"
...
OUT=$(vd 1)
has "версии: со своим стандартом сказано, что само ничего не сменилось" "осознанное действие"
...
# Прежние фикстуры её не имели, и на них проходила мутация «спутать собран_на_версии с
# собран» — то есть у правила § 9.3 п. 6 («записывается „собран на версии“, а не
# „версия“») машины не было, хотя тесты выглядели покрытием.
printf 'собран_на_версии: 1.0.0\nсобран: 2020-01-01\nимя: тест\n' > "$TVD/proj/.loreground"
OUT=$(vd 1)
has "версии: настоящая форма метки разбирается" "собрано на версии 1.0.0"
...
# Ворота у двух сторожей комплекта объявлены одни — значит совпадают и на краях.
rm -f "$TVD/proj/.loreground"; mkdir -p "$TVD/proj/.loreground"
OUT=$(vd 1); O2=$( (cd "$TVD/proj" && CLAUDE_PROJECT_DIR="$TVD/proj" CLAUDE_PLUGIN_ROOT="$KIT" python3 "$KIT/hooks/codex-freshness.py") )
if [ -z "$OUT" ] && [ -z "$O2" ]; then ok "ворота: папка с именем метки меткой не считается — обоими хуками"; else
  bad "ворота: хуки трактуют папку-метку по-разному (версии: «$OUT» · кодекс: «$O2»)"; fi
rm -rf "$TVD/proj/.loreground"
...
echo "── Умолчание режима подключения ──"
# Умолчание живёт в двух домах — норме (core.md § 9.3) и процедуре (init). Разъезд
# домов и есть тот дефект, ради которого этот тест написан: текст можно поправить в
# одном месте и не заметить второго.
CORE=../core.md
INIT=../../skills/init/SKILL.md
grep -q '^| \*\*Копия\*\* (по умолчанию)' "$CORE" \
  && ok "умолчание: стандарт объявляет копию" \
  || bad "умолчание: в таблице core.md § 9.3 копия не помечена умолчанием"
grep -q '(по умолчанию).*ядро остаётся в одном месте' "$CORE" \
  && bad "умолчание: стандарт всё ещё объявляет умолчанием указатель" \
  || ok "умолчание: стандарт указатель умолчанием не объявляет"
grep -q '^| \*\*Копия\*\* (умолчание)' "$INIT" \
  && ok "умолчание: навык объявляет копию" \
  || bad "умолчание: в таблице init копия не помечена умолчанием"
grep -q '^| \*\*Указатель\*\* (умолчание)' "$INIT" \
  && bad "умолчание: навык всё ещё объявляет умолчанием указатель" \
  || ok "умолчание: навык указатель умолчанием не объявляет"
grep -q 'по умолчанию \*\*копия\*\*' "$INIT" \
  && ok "умолчание: вопрос человеку предлагает копию" \
  || bad "умолчание: шаг 2 навыка предлагает не копию"
...
grep -q 'Версия ядра, на которой собран агент, записывается при сборке' "$CORE" \
  && ok "умолчание: требование записать версию сборки стоит в стандарте" \
  || bad "умолчание: § 9.3 ссылается на версию сборки, а требования записать её нет"
grep -q '^### 9.3.1' "$CORE" \
