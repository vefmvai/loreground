...
# ── Команда комплекта в среде хука: PATH здесь НЕ тот, что в терминале ──
# Папку `bin/` комплекта Claude Code дописывает в PATH только своей оболочке Bash.
# Процесс хука её не получает, а команда зовётся без оболочки (shell=False) — значит
# `lore-validate` из конфига режима «указатель» не находится никогда, и автопрогон
# у такого агента мёртв с первого дня.
#
# ПОЧЕМУ ЭТОТ ТЕСТ НЕ МОГ ПОЙМАТЬ ДЕФЕКТ РАНЬШЕ: все прогоны выше наследуют PATH
# терминала, где папка комплекта есть. Тест жил в тех же тепличных условиях, что и
# ручной прогон, и потому был слеп ровно к тому, чем ручной прогон от хука отличается.
# Здесь PATH назначается заново и содержит только python3 (без него не запустится сам
# хук) и системные утилиты (их зовёт обёртка). Обёртки комплекта в нём нет.
PBIN="$(mktemp -d)"; ln -s "$(command -v python3)" "$PBIN/python3"
HOOKPATH="$PBIN:/usr/bin:/bin"
# Предпосылка теста доказывается, а не предполагается: если обёртка вдруг лежит и
# в системной папке, тест ниже позеленел бы даром — тогда он честно снимается.
if PATH="$HOOKPATH" command -v lore-validate >/dev/null 2>&1; then
  ok "хук/PATH: пропущен — обёртка есть в системном PATH, предпосылка не воспроизводится"
else
  vwp() { printf '{"tool_name":"%s","cwd":"%s","tool_input":{"file_path":"%s"}}' "${1}" "$TVW" "${2}" \
          | (cd "$TVW" && PATH="$HOOKPATH" CLAUDE_PROJECT_DIR="$TVW" python3 "$KIT/hooks/validate-on-write.py"); }

  printf 'validate:\n  command: lore-validate knowledge\n' > "$TVW/config.yaml"
  OUT=$(vwp Write "$TVW/knowledge/Битая.md" | ctx)
  has "хук/PATH: команда комплекта найдена без PATH — прогон состоялся" "найдены проблемы"
  has "хук/PATH: отчёт валидатора приехал целиком" "FRONTMATTER"
  if echo "$OUT" | grep -qF "не найдена"; then
    bad "хук/PATH: обёртка комплекта не найдена — автопрогон в режиме «указатель» мёртв"
  else ok "хук/PATH: жалобы «команда не найдена» нет"; fi

  # Обратная сторона, и она важнее зелёного: подменять разрешено ТОЛЬКО имена, под
  # которыми в `bin/` комплекта правда лежит исполняемый файл. Чужая опечатка обязана
  # остаться видимой — иначе хук молча подставит похожее и соврёт про прогон.
  printf 'validate:\n  command: lore-validat knowledge\n' > "$TVW/config.yaml"
  OUT=$(vwp Write "$TVW/knowledge/Битая.md" | ctx)
  has "хук/PATH: опечатка в команде осталась видимой" "не найдена"

  # И третья сторона: своё имя не должно перебивать чужую рабочую команду. Если
  # команда в PATH есть, берётся она, а не одноимённый файл из комплекта.
  printf '#!/usr/bin/env bash\necho "чужой прогон"; exit 1\n' > "$PBIN/lore-validate"
  chmod +x "$PBIN/lore-validate"
  printf 'validate:\n  command: lore-validate knowledge\n' > "$TVW/config.yaml"
  OUT=$(vwp Write "$TVW/knowledge/Битая.md" | ctx)
  has "хук/PATH: команда из PATH сильнее одноимённой в комплекте" "чужой прогон"
  rm -f "$PBIN/lore-validate"
fi

# Кривая форма ключа `validate` обязана остаться ГРОМКОЙ. Проверка формы с тихим
# выходом выглядит аккуратнее и хуже: молчание хука по его устройству значит «чисто»,
# то есть поломка конфига превратилась бы в ложный зелёный.
printf 'validate: это строка, а не набор полей\n' > "$TVW/config.yaml"
OUT=$(vw Write "$TVW/knowledge/Битая.md" | ctx)
if [ -n "$OUT" ]; then ok "валидатор-хук: кривая форма validate названа вслух"
else bad "валидатор-хук: кривой конфиг проглочен молча — молчание читается как «чисто»"; fi

# ── Закон сужения: пропустить ФАЙЛ можно, пропустить ПРОГОН при нечитаемой границе — нет ──
# Все случаи ниже одной формы: хук решает «эта правка не про базу», хотя про базу он
# ничего не выяснил. Руками та же команда кричит кодом 2 — а хук молчал, и молчание по
# его же устройству значит «чисто». Стенд общий: база заведомо КРАСНАЯ, поэтому любое
# молчание здесь — пропущенный прогон, а не «нечего сказать».
# Заметка одной строкой — дом у этой заготовки один на оба набора хука ниже.
метка_бы() { printf -- '---\ntitle: %s\ntype: %s\nschema_version: "1.0"\n---\n%s\n' "$2" "$3" "$4" > "$1"; }
TVG="$(mktemp -d)"
mkdir -p "$TVG/Объект" "$TVG/Общее"
printf 'имя: стенд закона\n' > "$TVG/.loreground"
метка_бы "$TVG/Объект/Карточка.md"   "Карточка"  knowledge "Факты."
метка_бы "$TVG/Общее/00-index.md"    "Индекс"    moc       "- [[Карточка]]"
метка_бы "$TVG/Объект/Невидимка.md"  "Невидимка" такого-типа-нет "Тело."
vg() { printf '{"tool_name":"Write","cwd":"%s","tool_input":{"file_path":"%s"}}' "$TVG" "${1}" \
       | (cd "$TVG" && CLAUDE_PROJECT_DIR="$TVG" python3 "$KIT/hooks/validate-on-write.py"); }
граница_гов() { # $1 — что в base, $2 — имя проверки
  # %b, а не %s: подстановка со своими \n внутри. При %s они доехали бы в YAML
  # буквальными двумя знаками, конфиг склеился бы в одну строку, и тест зеленел бы
  # по неверной причине — граница ломалась бы разбором, а не тем, что проверяется.
  printf 'validate:\n  trust_layer: false\n  command: python3 %s/standard/validate.py\n  base:\n    mode: marker\n%b' "$KIT" "$1" > "$TVG/config.yaml"
  OUT=$(vg "$TVG/Объект/Карточка.md")
  if [ -n "$OUT" ]; then ok "$2"; else bad "$2 — хук промолчал, а молчание читается как «чисто»"; fi
}
граница_гов "    root: Обьект\n"                 "закон сужения: опечатка в root не глушит прогон"
граница_гов "    root: ~/такой-папки-нет\n"      "закон сужения: нераскрытая ~ в root не глушит прогон"
граница_гов "    root: .\n    exclude: [Объект, Общее]\n" "закон сужения: exclude, съевший базу, не глушит прогон"
граница_гов "    root: /\n"                      "закон сужения: корень файловой системы не глушит прогон"
# Обратная половина: на ИСПРАВНОЙ границе сужение обязано работать, иначе «починка»
# свелась бы к «прогонять всегда», а это не починка, а снятие проверки.
printf 'validate:\n  trust_layer: false\n  command: python3 %s/standard/validate.py\n  base:\n    mode: marker\n    root: .\n    exclude: [Общее]\n' "$KIT" > "$TVG/config.yaml"
OUT=$(vg "$TVG/Общее/00-index.md")
if [ -z "$OUT" ]; then ok "закон сужения: на исправной границе исключённый путь по-прежнему молчит"
else bad "закон сужения: сужение перестало работать вовсе — проверка снята, а не починена"; fi

# Первенство источников берётся у валидатора: названная в команде папка отменяет root.
printf 'validate:\n  trust_layer: false\n  command: python3 %s/standard/validate.py .\n  base:\n    mode: marker\n    root: Общее\n' "$KIT" > "$TVG/config.yaml"
OUT=$(vg "$TVG/Объект/Карточка.md")
if [ -n "$OUT" ]; then ok "закон сужения: слово команды сильнее root, как и у валидатора"
else bad "закон сужения: хук сузил по root, хотя валидатор обходит названную в команде папку"; fi

# Приведение значения берётся у валидатора же: свой `norm` не снимал кавычек.
printf 'validate:\n  trust_layer: false\n  command: python3 %s/standard/validate.py\n  base:\n    mode: %s\n    root: .\n    exclude: [Общее]\n' "$KIT" "'\"marker\"'" > "$TVG/config.yaml"
OUT=$(vg "$TVG/Общее/00-index.md")
if [ -z "$OUT" ]; then ok "закон сужения: mode в кавычках читается так же, как валидатором"
else bad "закон сужения: mode в кавычках хук и валидатор читают по-разному"; fi

# Симлинк-корень базы: валидатор такой корень обходит, значит и хук обязан его признать.
SYMDIR="$(mktemp -d)"; mkdir -p "$SYMDIR/снаружи"
метка_бы "$SYMDIR/снаружи/Вынесенная.md" "Вынесенная" knowledge "Факты."
ln -s "$SYMDIR/снаружи" "$TVG/Вынос"
printf 'validate:\n  trust_layer: false\n  command: python3 %s/standard/validate.py\n  base:\n    mode: marker\n    root: .\n' "$KIT" > "$TVG/config.yaml"
OUT=$(vg "$TVG/Вынос/Вынесенная.md")
if [ -n "$OUT" ]; then ok "закон сужения: файл за символической ссылкой не выпадает из базы"
else bad "закон сужения: симлинк увёл файл из проекта — хук замолчал навсегда"; fi
rm -rf "$SYMDIR"

# Регистр расширения: у правила один дом — тот же, что у обхода валидатора.
метка_бы "$TVG/Объект/Заглавная.MD" "Заглавная" knowledge "Факты."
OUT=$(vg "$TVG/Объект/Заглавная.MD")
if [ -n "$OUT" ]; then ok "закон сужения: .MD заглавными — та же заметка, что и .md"
else bad "закон сужения: .MD не считается заметкой, а валидатор её считает"; fi

# Имя команды звучит так, как его написал человек: до раскрытия `~`. Иначе он ищет в
# конфиге строку, которой там нет, а домашний путь заодно уезжает в контекст сессии.
printf 'validate:\n  command: ~/bin/заведомо-нет-такой-команды\n' > "$TVG/config.yaml"
OUT=$(vg "$TVG/Объект/Карточка.md" | ctx)
has "закон сужения: имя команды звучит как в конфиге, а не раскрытым" "«~/bin/заведомо-нет-такой-команды»"
case "$OUT" in *"$HOME"*) bad "закон сужения: домашний путь уехал в контекст сессии" ;;
               *) ok "закон сужения: домашний путь в контекст не уехал" ;; esac

# Битый config.yaml посреди сессии. Передача ответственности стартовому хуку законна
# только тогда, когда тот успеет сказать, — а он работает на старте сессии. Сломали
# конфиг в середине работы: автопрогон мёртв до конца сессии, и молчал бы об этом.
printf 'validate:\n  command: [не закрытый список\n' > "$TVG/config.yaml"
OUT=$(vg "$TVG/Объект/Карточка.md" | ctx)
has "закон сужения: неразбираемый config.yaml назван вслух" "не разбирается"
has "закон сужения: неразбираемый config.yaml — это не «чисто»" "не «чисто»"

# Код 1 без единой строки — отдельный ответ, а не «проблемы без подробностей».
printf '#!/usr/bin/env bash\nexit 1\n' > "$TVG/немой.sh"; chmod +x "$TVG/немой.sh"
printf 'validate:\n  command: ./немой.sh\n' > "$TVG/config.yaml"
OUT=$(vg "$TVG/Объект/Карточка.md" | ctx)
has "закон сужения: код 1 без вывода не выдаётся за находки в базе" "не напечатала НИЧЕГО"
rm -rf "$TVG"

# Сравнение «файл внутри папки» проверяется НАПРЯМУЮ, а не через событие, и это сказано
# вслух. Две его гарантии сегодня через событие недостижимы: корня файловой системы в
# `base.root` не бывает (такой обход отсекается раньше как «вне проекта»), а разные
# стороны симлинка приходят от хоста, которого на стенде нет. Прежняя редакция набора
# проверяла их «через хук» — и обе проверки оставались зелёными при снятой починке: до
# самой починки они не доходили. Проверка помощника прямо — честнее проверки через путь,
# который до него не идёт.
SUBCHECK=$(python3 - "$KIT" <<'PY'
import os, sys, tempfile, importlib.util
spec = importlib.util.spec_from_file_location("h", os.path.join(sys.argv[1], "hooks", "validate-on-write.py"))
m = importlib.util.module_from_spec(spec); spec.loader.exec_module(m)
беды = []
# Корень файловой системы: склейка разделителя давала `//` и не совпадала ни с чем.
if not m.under("/", "/private/tmp/x.md"):
    беды.append("under('/', файл) = False — корень ФС не признан родителем")
# Разные стороны символической ссылки: хост даёт одну, инструмент другую.
корень = tempfile.mkdtemp(); настоящая = os.path.join(корень, "real"); ссылка = os.path.join(корень, "link")
os.makedirs(os.path.join(настоящая, "База")); os.symlink(настоящая, ссылка)
файл = os.path.join(настоящая, "База", "з.md"); open(файл, "w").close()
if not m.under(ссылка, файл):
    беды.append("under(симлинк-проект, настоящий-файл) = False — хук замолчал бы навсегда")
if not m.under(настоящая, os.path.join(ссылка, "База", "з.md")):
    беды.append("under(настоящий-проект, файл-через-симлинк) = False")
# Обратная сторона: чужая папка родителем не считается, иначе «сомнение в пользу да»
# превратилось бы в «всё под всем».
if m.under(os.path.join(настоящая, "База"), os.path.join(настоящая, "снаружи.md")):
    беды.append("under() признал родителем чужую папку — сужение перестало что-либо значить")
print("; ".join(беды))
PY
)
if [ -z "$SUBCHECK" ]; then ok "сравнение «внутри папки»: корень ФС, обе стороны симлинка, чужая папка"
else bad "сравнение «внутри папки»: $SUBCHECK"; fi

# ── Граница базы в раскладке «по метке»: её задаёт config.yaml, а не слова команды ──
# В этой раскладке имени папки в команде нет вовсе (§ 9.3 п. 8), поэтому разбор слов
# не распознавал ничего и хук прогонял базу на КАЖДОЙ правке — включая пути, которые
# проект исключил из базы явно: рабочие папки инструментов и личный слой `my/`.
# Каждый такой прогон заведомо не о базе.
TVB="$(mktemp -d)"
mkdir -p "$TVB/Объект" "$TVB/Общее" "$TVB/Рабочее"
printf 'имя: стенд границы\n' > "$TVB/.loreground"
метка_бы "$TVB/Объект/Карточка.md" "Карточка" knowledge "Факты."
метка_бы "$TVB/Общее/00-index.md"  "Индекс"   moc       "- [[Карточка]]"
# База заведомо КРАСНАЯ: без этого молчание хука ничего не доказывает — оно было бы
# неотличимо от «прогон состоялся и всё чисто».
метка_бы "$TVB/Объект/Невидимка.md" "Невидимка" такого-типа-нет "Тело."
cat > "$TVB/config.yaml" <<YML
validate:
  trust_layer: false
  command: python3 $KIT/standard/validate.py
  base:
    mode: marker
    root: .
    ...
    exclude: [Рабочее]
YML
vb() { printf '{"tool_name":"%s","cwd":"%s","tool_input":{"file_path":"%s"}}' "${1}" "$TVB" "${2}" \
       | (cd "$TVB" && CLAUDE_PROJECT_DIR="$TVB" python3 "$KIT/hooks/validate-on-write.py"); }

# Половина первая: правка в исключённом пути прогон НЕ запускает.
printf 'рабочий файл фреймворка, не заметка\n' > "$TVB/Рабочее/rabota.md"
OUT=$(vb Write "$TVB/Рабочее/rabota.md")
if [ -z "$OUT" ]; then ok "хук/метка: правка в исключённом пути прогон не запускает"
else bad "хук/метка: прогнал базу из-за правки в exclude — граница из config.yaml не прочитана"; fi

# Половина вторая, без которой первая ничего не стоит: правка В БАЗЕ прогон запускает.
# Иначе «молчание» выше доказывало бы лишь то, что хук умер целиком.
OUT=$(vb Write "$TVB/Объект/Карточка.md" | ctx)
has "хук/метка: правка в базе прогон запускает" "найдены проблемы"

# И третья: файл вне корня обхода тоже мимо. Корень здесь `.`, поэтому проверяется
# сдвигом корня — а не выдумыванием пути за пределами проекта, куда агент не пишет.
printf 'validate:\n  trust_layer: false\n  command: python3 %s/standard/validate.py\n  base:\n    mode: marker\n    root: Объект\n    index: ../Общее/00-index.md\n' "$KIT" > "$TVB/config.yaml"
OUT=$(vb Write "$TVB/Общее/00-index.md")
if [ -z "$OUT" ]; then ok "хук/метка: файл вне корня обхода прогон не запускает"
else bad "хук/метка: прогнал базу из-за файла вне корня обхода"; fi
rm -rf "$TVB"
...
# `--` перед форматом обязателен: bash считает ведущие дефисы своим ключом и падает
# «invalid option», а фикстура получается ПУСТОЙ — проверка ниже зеленеет на пустом файле.
...
# СЛОМАННЫЙ МАНИФЕСТ — НЕ «комплекта нет». Прежняя редакция этих двух проверок
# ТРЕБОВАЛА молчания с доводом «комплекта фактически нет, сравнивать не с чем» — и тем
# запирала дефект: комплект есть, он сломан, а сторож молчит, и молчание сторожа по его
# устройству значит «версии совпали». Тест, требующий молчания там, где машина не смогла
# проверить, — не проверка, а цемент. Законное молчание ровно одно: манифеста нет вовсе,
# и оно проверяется ниже отдельно.
printf 'собран_на_версии: 1.0.0\nимя: тест\n' > "$TVD/proj/.loreground"
for MANIFEST in '["не объект"]' '{"name":"loreground","version":123}' \
                '{"name":"loreground"}' '{"name":"loreground","version":"  "}' 'не json{'; do
  printf '%s\n' "$MANIFEST" > "$TVD/kit/.claude-plugin/plugin.json"
  OUT=$(vd 1); RC=$?
  if [ -n "$OUT" ] && [ "$RC" -eq 0 ]; then ok "версии: сломанный манифест назван вслух (${MANIFEST})"; else
    bad "версии: на манифесте «${MANIFEST}» сторож промолчал (rc=$RC) — молчание читается как «версии совпали»"; fi
done
OUT=$(vd 1); has "версии: сломанный манифест — это не «версии совпали»" "НЕ выполняется"
# А вот отсутствие манифеста — законное молчание: комплекта на машине правда нет.
rm -f "$TVD/kit/.claude-plugin/plugin.json"
OUT=$(vd 1)
if [ -z "$OUT" ]; then ok "версии: отсутствие манифеста молчит — комплекта нет, сравнивать не с чем"; else
  bad "версии: ругается на штатное состояние «комплекта на машине нет»"; fi
printf '{"name":"loreground","version":"9.9.9"}\n' > "$TVD/kit/.claude-plugin/plugin.json"

# Потолок автопрогона объявлен в `hooks.json` ЯВНО и с запасом над собственным потолком
# скрипта. Иначе прогон снимает среда своим умолчанием, снятый процесс ничего не печатает,
# и «медленно» превращается в тишину, а тишина хука значит «чисто». Это то же правило,
# что и про `PATH`: среду назначают, а не заимствуют, — поэтому у него есть машина.
TIMECAP=$(python3 - "$KIT" <<'PY'
import json, os, re, sys
kit = sys.argv[1]
d = json.load(open(os.path.join(kit, "hooks", "hooks.json"), encoding="utf-8"))
запись = d["hooks"]["PostToolUse"][0]["hooks"][0]
объявлен = запись.get("timeout")
свой = re.search(r"timeout=(\d+)", open(os.path.join(kit, "hooks", "validate-on-write.py"), encoding="utf-8").read())
свой = int(свой.group(1)) if свой else None
if объявлен is None: print("в hooks.json нет потолка у PostToolUse — его назначит среда своим умолчанием")
elif свой is None:   print("в скрипте не нашёлся собственный потолок прогона")
elif объявлен <= свой: print("потолок в hooks.json (%s) не больше собственного (%s) — ветка «не уложился» достанется среде" % (объявлен, свой))
PY
)
if [ -z "$TIMECAP" ]; then ok "потолок автопрогона назначен явно и с запасом над собственным"
else bad "потолок автопрогона: $TIMECAP"; fi
